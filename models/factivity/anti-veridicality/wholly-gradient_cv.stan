...
transformed parameters {
  ...
  vector<lower=0, upper=1>[N_data_tr] v_tr; // probability of projection (training data)
  vector<lower=0, upper=1>[N_data_te] v_te; // probability of projection (test data)
  ...
  vector<lower=0, upper=1>[N_data_tr] a_tr; // probability of an anti-veridicality inference (training data)
  vector<lower=0, upper=1>[N_data_te] a_te; // probability of an anti-veridicality inference (test data)
  ...
  for (i in 1:N_data_tr) {
    ...
    a_tr[i] = inv_logit(alpha[predicate_tr[i]] + epsilon_alpha[participant_tr[i]]);
  ...
  for (i in 1:N_data_te) {
    ...
    a_te[i] = inv_logit(alpha[predicate_te[i]] + epsilon_alpha[participant_te[i]]);
